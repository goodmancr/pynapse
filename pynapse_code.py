...
class StartTrial: # StateID = ?
    def s_State_enter():
        ...
        if p_Metric.Alcohol.toInt() == 10:
            Cond = 2
            
        # if we reach 20 water trials, switch to alcohol trials
        if p_Metric.Water.toInt() == 10:
            Cond = 1

        if (p_Metric.ntrials.toInt() != 21):
            if Cond == 1:
...
class AlcoholTrial: # StateID =?
    ...
    def s_State_enter():
        p_Rig.RSpout.turnOn()
        p_State.setTimeout(5, AlcoholSipperOff)
...
class WaterTrial: # StateID = ?
    # wait 1 sec before sending spout out
    # spout out for 15 seconds
    # counts lick if LLick is triggered
    def s_State_enter():
        time.sleep(1)
        p_Rig.LSpout.turnOn()
        p_State.setTimeout(5, WaterSipperOff)

    # if LLick is triggered,
    #       increment LLickNum
    def s_LLick_rise():
